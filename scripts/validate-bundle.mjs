...
const baseFoods = readData('foods.json').foods
...
function recipePerServing(r) {
  ...
  return { kcal: kcal / s, proteinG: prot / s }
}
function mealMacros(m, dayLabel) {
  const at = `week ${dayLabel}`
  if (m.recipeId) {
    const r = recipesById.get(m.recipeId)
    if (!r) { err(`${at} : recipeId introuvable « ${m.recipeId} ».`); return { kcal: 0, proteinG: 0 } }
    const per = recipePerServing(r)
    const q = m.portions ?? 1
    return { kcal: per.kcal * q, proteinG: per.proteinG * q }
  }
  if (m.foodId) {
    const f = foodsById.get(m.foodId)
    if (!f) { err(`${at} : foodId introuvable « ${m.foodId} ».`); return { kcal: 0, proteinG: 0 } }
    if (!isNum(m.grams)) { err(`${at} : grams requis pour foodId « ${m.foodId} ».`); return { kcal: 0, proteinG: 0 } }
    const factor = m.grams / 100
    return { kcal: f.per100g.kcal * factor, proteinG: f.per100g.proteinG * factor }
  }
  if (m.estimated) {
    if (!isNum(m.estimated.kcal) || !isNum(m.estimated.proteinG)) err(`${at} : estimated.kcal/proteinG requis.`)
    return { kcal: m.estimated.kcal ?? 0, proteinG: m.estimated.proteinG ?? 0 }
  }
...
let dayComputed = [] // {label, kcal, proteinG, meals:[{desc,kcal,prot}]}
if (bundle.week) {
  ...
  for (const d of days ?? []) {
    ...
    for (const m of meals) {
      ...
      kcal += mm.kcal; prot += mm.proteinG
      details.push({ desc: m.recipeId ?? m.foodId ?? (m.estimated ? 'estimé' : '?'), kcal: mm.kcal, prot: mm.proteinG })
    }
    dayComputed.push({ label: d.label, kcal, proteinG: prot, meals: details })
  }
...
console.log(`Contenu : ${(bundle.foods ?? []).length} aliment(s), ${(bundle.recipes ?? []).length} recette(s), ` +
...
if (bundle.week) {
  ...
  for (const d of dayComputed) console.log(`  ${d.label.padEnd(9)} ${d.kcal.toFixed(0).padStart(5)} kcal  P${d.proteinG.toFixed(0)}`)
}
